...
export class InvitationResolver {
    ...
    @Mutation(() => Invitation)
    async createInvitation(@Arg("data", () => InvitationCreateInput) data: InvitationCreateInput): Promise<Invitation> {
        ...
        return await datasource.manager.transaction(async transactionalEntityManager => {
            ...
            try {
                ...
                await emailService.sendInvitationEmail(data.email, group.name, newInvitation.token);
            } catch (error: any) {
    ...
    async validateInvitationToken(@Arg("token") token: string): Promise<Group | null> {
    ...
    async acceptInvitation(
        ...
        userGroup.user = { id: userId } as any;
