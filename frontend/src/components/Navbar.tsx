    ...
    function MobileMenuItem({ item }: { item: typeof items[0] }) {
        ...
        return (
            <SidebarMenuItem className="p-0">
                <SidebarMenuButton asChild className="w-full hover:bg-[#FFFBFF]/10 rounded-lg transition-colors duration-200">
                    <Link 
                        to={item.path} 
                ...
                <div className="flex items-center gap-8">
                    {items.slice(1).map((item) => (
                        <Link
                            ...
                            className="flex items-center gap-2 hover:text-primary transition-colors"
                        >
                            <span className="text-[#FFFBFF] text-3xl">{item.title}</span>
                        </Link>
                    ))}
                </div>
            </nav>
                    ...
                    <div className="flex items-center w-full justify-between">
                        ...
                        <div className="ml-2">
                            <Link to="/dashboard" className="flex items-center gap-2">
                                <img src="/giftwhisper-logo.svg" alt="Logo" className="w-[50.9px] h-[47px]" />
                            </Link>
                        </div>
                    </div>
