...
//             <button className="add-button"><IoAdd size={20} color="white" /></button>
...
//           </div>
//           <button className="chat-button" type="submit"><FaLocationArrow color="white" size={16} /></button>
//         </form>
//       </article>
//     </>
//   )
  ...
  return (
    <>
      <article className="h-screen flex flex-col">
        ...
        <section className="flex flex-col flex-1 overflow-x-auto p-2 space-y-2">
          {messages.map((message) => {
            return (
              <div key={message.id} className={`flex flex-col max-w-[70%] ${message.author !== 'me' ? 'self-end' : ''}`}>
                <div className={`flex items-center gap-1 ${message.author !== 'me' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <p className={`pb-1.5 ${message.author !== 'me' ? 'text-right' : ''}`}>{message.author}</p>
                </div>
